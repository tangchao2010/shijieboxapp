...
export default class RefreshListView extends Component {
  ...
  /**
   * propTypes是我们给RefreshListView组件定义的给外部调用的方法，方法类型需要使用PropTypes来指定
   * */
  static propTypes = {
    ...
    createEmptyView() {
        return (
            <View style={{flex: 1, justifyContent: 'center', backgroundColor: 'red'}}>
                <Text style={{flex: 1, fontSize: 14, color: '#666',alignSelf: 'center'}}>还没有数据哦</Text>
            </View>
        );
    }
  ...
  shouldStartHeaderRefreshing() {
    if (this.state.footerState === RefreshState.refreshing || this.state.isHeaderRefreshing || this.state.isFooterRefreshing) {
      return false;
    }
    return true;
  }
...
{/*<FlatList
        ...
        ListEmptyComponent={() => {
            if (false) {
                return (
                    <TouchableOpacity
                        style={{ flex: 1 , height: Layout.window.height - 200, backgroundColor: Colors.screenBackgroundColor, alignItems: 'center', justifyContent: 'center' }}
                        onPress={() => this._refresh()}
                        activeOpacity={0.6}>
                        <Text style={{ textAlign: 'center', color: '#666', fontSize: 14 }}> 点击重试</Text>
                    </TouchableOpacity>
                )
            } else {
                return <View style={{ height: Layout.window.height - 200, backgroundColor: Colors.screenBackgroundColor, alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{color: '#666', fontSize: 14}}>暂无数据</Text>
                </View>
            }
        }}
        ListFooterComponent={this._renderFooter}
